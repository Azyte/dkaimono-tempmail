...
export function AmPremiumModal({ isOpen, onClose, onSuccessCreated }: AmPremiumModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/85 p-2 sm:p-4 backdrop-blur-lg animate-in fade-in duration-200">
      <div className="relative max-h-[96dvh] sm:max-h-[92dvh] w-full max-w-2xl overflow-hidden rounded-2xl sm:rounded-3xl border border-slate-800 bg-slate-900/95 shadow-2xl flex flex-col">
        ...
        {/* Modal Body (Scrollable with bottom padding) */}
        <div className="flex-1 overflow-y-auto custom-scrollbar p-3.5 sm:p-5 space-y-4 pb-16 sm:pb-6">
                ...
                {results.map((acc, idx) => {
                  ...
                  if (acc.serviceType === 'video_clipper') {
                    return (
                      <div
                        key={acc.id || idx}
                        className="rounded-2xl border border-rose-500/40 bg-slate-950/95 p-3.5 sm:p-4 text-xs space-y-3.5 shadow-xl"
                      >
                        ...
                        <div className="space-y-1.5">
                          ...
                          <div className="space-y-1.5">
                            {(acc.viralTitles || [acc.videoTitle]).map((title: string, tIdx: number) => (
                              ...
                              >
                                <span className="text-[11px] text-slate-200 font-medium line-clamp-1 break-all">{title}</span>
                                <button
                                  type="button"
                                  onClick={() => handleCopy(title, `title_${tIdx}`)}
                                  className="text-slate-400 hover:text-cyan-300 shrink-0 p-1"
                                >
                        ...
                        <div className="space-y-1">
                          ...
                          <div className="flex items-center justify-between gap-2 rounded-xl bg-slate-900 px-3 py-2 border border-slate-800">
                            <span className="text-[11px] text-cyan-300 font-mono line-clamp-1 break-all">{acc.viralHashtags}</span>
                            <button
                              type="button"
                        ...
                        <div className="space-y-1">
                          ...
                          <div className="flex items-center justify-between gap-2 rounded-xl bg-slate-900 px-3 py-2 border border-slate-800">
                            <span className="text-[11px] text-amber-200 line-clamp-1 break-all">{acc.pinnedCommentCta}</span>
                            <button
                              type="button"
                        ...
                        <div className="pt-1">
                          <button
                            ...
                            className="w-full flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-purple-600 via-rose-600 to-amber-600 py-3 sm:py-3.5 px-3 text-xs sm:text-sm font-bold text-white shadow-xl hover:from-purple-500 hover:to-amber-500 active:scale-[0.98] transition-all"
                          >
                            <Sparkles className="h-4 w-4" />
                            <span>🎬 Buka Video Studio (Auto Split-Screen & Anti-Copyright)</span>
                          </button>
                        </div>
                      </div>
                    );
